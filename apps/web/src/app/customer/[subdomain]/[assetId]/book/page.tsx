...
type CustomerBookingFormData = z.infer<typeof BaseCustomerBookingObjectSchema> &
  Record<string, any>;
...
export default function CustomerBookingPage() {
  ...
  const {
    ...
    isLoading,
    error: queryError,
  } = client.assets.getAssetDetailsBySubdomain.useQuery({
  ...
  // Forms for asset
  const { data: formsResponse, isLoading: isLoadingForms } =
    client.settings.form.getFormsForAssetPublic.useQuery({
  ...
  // Blocked dates
  const { data: blockedDatesResponse } =
    client.blockedDates.getBlockedDatesForAssetPublic.useQuery({
  ...
  const blockedDates: BlockedDateRange[] =
    blockedDatesResponse?.status === 200
  ...
  const availableAddons =
    addonsResponse?.status === 200 ? addonsResponse.body : [];
  ...
  const availableTaxFees =
    taxFeesResponse?.status === 200 ? taxFeesResponse.body : [];
  ...
  const { mutate: createBooking, isPending } =
    client.booking.customerCreateBooking.useMutation();

  // Watch selected dates
  const selectedDates = form.watch("dateRange");

  // Fetch all rates for asset
  const { data: ratesResponse, isLoading: isRateLoading } =
    client.rates.getPublicRates.useQuery({
  ...
  const allRates =
    ratesResponse?.status === 200
      ? ratesResponse.body.data.map((d) => d.rate)
      : [];
  ...
  const { rateSegments, totalPrice, numberOfNights } = useMemo(() => {
    ...
    while (current < selectedDates.to) {
      ...
      const dayRate =
        allRates
          .filter((r) => {
            if (!r.startMonth || !r.startDay || !r.endMonth || !r.endDay)
              return false;
      ...
      <div className='min-h-screen flex items-center justify-center bg-background'>
        <Card className='max-w-md'>
          <CardContent className='pt-6 text-center'>
            ...
            <Button asChild>
              <Link href={`/customer/${subdomain}`}>Back to Assets</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
                      ...
                      <>
                        {rateSegments.map((seg, i) => (
                          <p key={i} className='text-green-800 font-medium'>
                            {seg.days} night{seg.days !== 1 ? "s" : ""} at $
                            {seg.pricePerUnit}/night = ${seg.subtotal}
                          </p>
                        ))}
                        {Object.entries(selectedAddons).map(
                          ([addonIdStr, qty]) => {
                            const addon = availableAddons.find(
                              (a: any) => a.id === Number(addonIdStr),
                            );
                            if (!addon || qty === 0) return null;
                            const price = Number(addon.price);
                            const addonTotal =
                              addon.billingType === "per_unit"
                        ...
                        {(() => {
                          ...
                          const sorted = [...availableTaxFees].sort(
                            (a: any, b: any) => a.sortOrder - b.sortOrder,
                          );
                          const netItems = sorted.filter(
                            (tf: any) => tf.calculationBasis === "net",
                          );
                          const grossItems = sorted.filter(
                            (tf: any) => tf.calculationBasis === "gross",
                          );
                          const computeTf = (tf: any, base: number) => {
                            ...
                            if (tf.calculationType === "percentage")
                              ...
                              const units = tf.maxUnits
                                ? Math.min(numberOfNights, tf.maxUnits)
                                : numberOfNights;
                              amount = rate * units;
                            } else amount = rate;
                            if (tf.minAmount)
                              amount = Math.max(amount, Number(tf.minAmount));
                            if (tf.maxAmount)
                              amount = Math.min(amount, Number(tf.maxAmount));
                          ...
                          const grandTotal =
                            subtotal + netTaxTotal + grossTaxTotal;
                          return (
                            <>
                              {taxLines.map((line, i) => (
                                <p
                      ...
                      <div key={formData.form.id} className='space-y-4'>
                        ...
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                          {formData.fields.map((field) => {
                            const key =
                              `form_${formData.form.id}_${field.id}` as any;
                            return (
                              <div
                                key={field.id}
                                className={
